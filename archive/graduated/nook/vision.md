...
| **Nook Streaks (F-17)** | Consecutive-day interaction counter shared between two friends on both home screen widgets. Any widget action (duet, pet feed, game move, knock) maintains the streak. Streak breaks when the pet gets sad. | The variable-reward retention loop the ambient design lacks — gives users a legible daily pull-back trigger without notification spam. | MVP-adjacent (Phase 1) |
| **Mood Capsule (F-20)** | Once-daily emoji mood stamp that auto-appears on close friends' home screen widgets for 24 hours then vanishes. No text, no reply pressure. | Ephemeral urgency (disappears in 24h) drives daily habit without algorithmic pressure. Most low-friction feature in the set. | MVP-adjacent (Phase 1) |
| **Room of the Week (F-16)** | Weekly community vote selects top Room per Hall theme. Winner gets their Room pinned at Hall lobby + an exclusive limited-edition furniture piece as prize. | Recurring competition loop gives users a concrete reason to redecorate regularly. Drives Hall traffic. | Phase 2 (with Halls launch) |
| **Hall Campfire (F-18)** | Async 10-second audio embers scattered inside Community Halls. Drop a short voice clip or sound snippet; visitors tap glowing embers to hear them; embers fade after 24h. | Solves the Hall ghost-town concurrency problem — Halls feel populated through async audio even when concurrent users are low. | Phase 2 (with Halls launch) |
| **Pixel Photo Booth (F-22)** | In-app camera filter pixelating a selfie into a Nookie-style sprite overlay inside your Room background. Share directly to Instagram/TikTok Stories with embedded `nook.me/user` deep-link. | Biggest external virality driver — identity content that makes TikTok viewers curious about the app without requiring them to understand it first. | Phase 2 |
| **Nookie Trading Cards (F-19)** | Collectible card of your Nookie in a rare pose + Room aesthetic background. Friends pin your card in their Room as wall art; display your friends' collection on your shelves. | Social proof + collection loop. Rare cards become a flex; collecting friends' cards creates attachment to specific people. | Phase 3 (v1.0) |
| **Secret Knock Code (F-23)** | Each friendship pair shares a unique rhythmic haptic pattern — their "secret handshake." Vibe-Knocks pulse in your personal friendship rhythm. Friends can co-customize the pattern together. | Transforms Vibe-Knocks from generic haptic pings into intimate, personalized signals. Attachment mechanic that increases switching cost. | Phase 3 (v1.0) |
| **Nook Neighborhood (F-21)** | Private group Room for up to 8 friends. Each member owns a "house" on a shared pixel-art block with a communal courtyard where all Nookies passively hang out. | Solves cold-start Hall loneliness — your neighborhood is always populated by the 8 people you actually care about. | Future (post v1.0) |
